      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-3xl bg-white p-8 shadow-sm shadow-slate-200">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.28em] text-slate-500">Instructor</p>
        ...
        <section className="grid gap-6 lg:grid-cols-3">
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm font-medium text-slate-600">Courses Teaching</p>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <p className="text-sm font-medium text-slate-600">Avg. Student Rating</p>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-sm">
            <div className="flex items-center justify-between gap-4 border-b border-slate-200 pb-4">
              <div>
                ...
                <p className="text-sm text-slate-600">Courses you are currently teaching.</p>
              </div>
              <Link
                href="/courses/create"
          ...
          <div className="space-y-6">
            <div className="rounded-3xl bg-white p-6 shadow-sm">
              <div className="border-b border-slate-200 pb-4">
                ...
                <p className="text-sm text-slate-600">Recent alerts for your account.</p>
              </div>
                ...
                {notifications.length === 0 ? (
                  ...
                  notifications.slice(0, 5).map((notification) => (
                    <div key={notification.id} className="rounded-3xl border border-slate-200 p-4">
                      ...
                      <p className="mt-2 text-xs text-slate-500">{new Date(notification.created_at).toLocaleDateString()}</p>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="rounded-3xl bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 space-y-4">
                {dashboardData?.recent_activities?.length === 0 ? (
                  <p className="text-sm text-slate-600">No recent student activity.</p>
                ) : (
                  dashboardData.recent_activities.map((activity: any, index: number) => (
                    <div key={index} className="rounded-3xl border border-slate-200 p-4">
                      ...
                      <p className="mt-2 text-xs text-slate-500">{new Date(activity.created_at).toLocaleDateString()}</p>
                    </div>
                  ))
                )}
              </div>
