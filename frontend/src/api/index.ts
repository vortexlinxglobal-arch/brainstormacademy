...
const getBackendUrl = () => {
  ...
  if (backendUrl.includes(".supabase.co")) {
    throw new Error(
      "NEXT_PUBLIC_BACKEND_URL must point to the backend API, not the Supabase project URL.",
    );
  }
  return backendUrl;
};
...
async function fetchBackend(endpoint: string, options: RequestInit = {}) {
  ...
  try {
    ...
    data = { error: "Invalid JSON response", rawText };
  }

  if (!response.ok) {
    const msg = data?.error || `Backend request failed: ${response.status}`;
    const err: any = new Error(msg);
    // Attach additional details for richer diagnostics in calling code
    err.status = response.status;
    err.responseBody = data;
    err.rawResponseText = rawText;
    // Log full backend response to console for debugging during development
...
class ApiClient {
  ...
  private async request(endpoint: string, options: RequestInit = {}) {
    ...
    const apiEndpoint = endpoint.startsWith("/v1")
      ? endpoint
      : `/v1${endpoint}`;
...
export const auth = {
  ...
  onAuthStateChange(callback: (event: string, session: any) => void) {
    ...
    if (!client) {
      console.warn(
        "Supabase auth state is unavailable because the public Supabase client is not configured.",
      );
