...
app.get("/v1/success-ticker", async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("students")
      .select(
        "id, first_name, last_name, enrollment_status, updated_at, enrollments!left(trade_id, trades(name))",
      )
    ...
    const messages = (data || []).map((student) => {
      const name =
        `${student.first_name || "A learner"} ${student.last_name || ""}`.trim();
      const tradeName =
...
app.post("/v1/admissions/review", async (req, res) => {
  try {
    ...
    if (error) return respond(res, { error: error.message }, 400);
    return respond(res, {
      data: { success: true, message: "Application reviewed successfully" },
    });
  } catch (error) {
    return respond(res, { error: error.message }, 500);
  }
...
app.post("/v1/admissions/letter", async (req, res) => {
  try {
    ...
    if (!application_id || !letter_type) {
      return respond(
        res,
        { error: "application_id and letter_type are required" },
        400,
      );
    }

    const { data: letterId, error } = await supabase.rpc(
...
app.get("/v1/trades", async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return respond(
        res,
        { error: error.message || "Unable to load trade programs" },
        500,
      );
    }
...
app.get("/v1/trades/courses", async (req, res) => {
  try {
    ...
    if (!tradeCode)
      return respond(res, { error: "trade_code parameter required" }, 400);
...
app.post("/v1/admin/students", async (req, res) => {
  try {
    ...
    const { data: student, error: studentError } = await supabase
      .from("students")
      .select(
        "id, user_id, registration_number, first_name, last_name, enrollment_status",
      )
...
app.put("/v1/trades", async (req, res) => {
  try {
    ...
    if (error) return respond(res, { error: error.message }, 400);
    return respond(res, {
      data: { success: true, message: "Trade updated successfully" },
    });
  } catch (error) {
    return respond(res, { error: error.message }, 500);
  }
...
app.put("/v1/students", async (req, res) => {
  try {
    ...
    const { contact, guardian_contact, address, academic_background } =
      req.body;
...
app.get("/v1/students/modules", async (req, res) => {
  try {
    ...
    if (!courseId)
      return respond(res, { error: "course_id parameter required" }, 400);
...
app.post("/v1/students/progress", async (req, res) => {
  try {
    ...
    if (!course_enrollment_id || !module_id) {
      return respond(
        res,
        { error: "course_enrollment_id and module_id are required" },
        400,
      );
    }
...
app.get("/v1/staff/id-card", async (req, res) => {
  return respond(res, {
    data: {
      message:
        "Staff ID card endpoint is not yet implemented. This route is reserved for future dashboard integration.",
    },
  });
});
...
app.get("/v1/staff/remuneration", async (req, res) => {
  return respond(res, {
    data: {
      message:
        "Staff remuneration endpoint is not yet implemented. Configure payroll tables or RPCs to return compensation details.",
    },
  });
});
...
app.get("/v1/staff/finance", async (req, res) => {
  return respond(res, {
    data: {
      message:
        "Staff finance summary endpoint is not yet implemented. Add reporting RPCs for balances, allowances, and expenses.",
    },
  });
});
...
app.post("/v1/staff", async (req, res) => {
  try {
    ...
    if (staffError)
      return respond(res, { error: "Staff profile creation failed" }, 400);
    ...
    return respond(res, {
      data: {
        ...
        staff,
        message:
          "Staff member registered successfully. Please check your email for verification.",
      },
    });
  } catch (error) {
    return respond(res, { error: error.message }, 500);
...
app.get("/v1/staff/department", async (req, res) => {
  try {
    ...
    if (!code)
      return respond(res, { error: "department code parameter required" }, 400);
...
app.post("/v1/admin/business-center/items", async (req, res) => {
  try {
    ...
    if (!item_name || quantity === undefined || unit_cost === undefined) {
      return respond(
        res,
        { error: "item_name, quantity, and unit_cost are required" },
        400,
      );
    }
  ...
  try {
    ...
    if (transactions && transactions.length > 0) {
      return respond(
        ...
        {
          error:
            "Cannot delete item with existing transactions. Archive or deactivate instead.",
        },
        409,
      );
    }
...
app.get("/v1/admin/business-center/transactions", async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("business_center_inventory_transactions")
      .select(
        "*, inventory_item:business_center_inventory_items(item_name, sku), branch:business_center_branches(id, name)",
      )
      .order("created_at", { ascending: false });
  ...
  try {
    ...
    if (
      ...
      return respond(
        ...
        {
          error:
            "inventory_item_id, transaction_type, quantity, and unit_cost are required",
        },
        400,
      );
    }
  ...
  try {
    ...
    if (!topic || !scheduled_date || !location || !participants) {
      return respond(
        ...
        {
          error:
            "topic, scheduled_date, location, and participants are required",
        },
        400,
      );
    }
...
app.get("/v1/admin/payments", async (req, res) => {
  try {
    ...
    const { data, error } = await supabase
      .from("payments")
      .select(
        "*, students(id, first_name, last_name), enrollments(id, trade_id, trade:trades(name))",
      )
...
app.post("/v1/admin/payments", async (req, res) => {
  try {
    ...
    if (!student_id && student_email) {
      ...
      if (studentError)
        return respond(res, { error: studentError.message }, 400);
      if (!student)
        return respond(
          res,
          { error: "Student not found for provided email" },
          404,
        );
    ...
    if (!enrollment_id) {
      ...
      if (enrollmentError)
        return respond(res, { error: enrollmentError.message }, 400);
      if (!enrollment)
        return respond(
          res,
          { error: "No enrollment found for the selected student" },
          404,
        );
  ...
  try {
    ...
    if (!email || !password || !full_name || !date_of_birth || !trade_code) {
      return respond(
        ...
        {
          error:
            "Missing required fields: email, password, full_name, date_of_birth, trade_code",
        },
        400,
      );
    }
    ...
    if (tradeErr) {
      ...
      if (
        tradeErr.message?.includes("Could not find the table") ||
        tradeErr.code === "PGRST205"
      ) {
    ...
    const emailRedirectTo =
      req.body.email_redirect_to || req.body.emailRedirectTo;
    const defaultAppUrl =
    ...
    if (authError) {
      ...
      console.error("Supabase signUp error:", {
        ...
        fullError: authError,
        objectKeys: Object.keys(authError),
      ...
      // Fallback: create user with service role when signUp fails unexpectedly.
      if (authError.code === "unexpected_failure") {
        console.warn(
          "Falling back to admin.createUser due to unexpected signUp failure.",
        );
        ...
        if (createError) {
          ...
          console.error("Supabase admin.createUser error:", {
            ...
            ownProps: Object.getOwnPropertyNames(createError),
          });
          return respond(
            res,
            {
              error: `Database error saving new user: ${createMessage}`,
            },
            400,
          );
        }
    ...
    if (trade) {
      ...
      console.warn(
        "Skipping trade enrollment because trade lookup is unavailable.",
      );
    }
...
app.get("/v1/notifications", async (req, res) => {
  try {
    ...
    if (error) {
      return respond(
        res,
        { error: error.message || "Unable to fetch notifications" },
        500,
      );
    }
